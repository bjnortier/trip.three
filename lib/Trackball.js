  ...
  constructor(scene, rawEventGenerator) {
    ...
    function updateCamera() {
      if (scene.mode === 'perspective') {
        ...
        scene.camera.position.set(
          lookAt.x + r*Math.sin(target.elevation) * Math.cos(target.azimuth),
          lookAt.y + r*Math.sin(target.elevation) * Math.sin(target.azimuth),
          lookAt.z + r*Math.cos(target.elevation));
      }
    ...
    function drag(event, eventPosition, mouseDownPosition) {
      ...
      if (state === 'panning') {
        ...
        const factor = (scene.mode === 'perspective') ? 100 : 200;
        dPos.multiplyScalar(Math.sqrt(target.distance)/factor);
