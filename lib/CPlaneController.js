...
class CPlaneController extends Controller {
  ...
  constructor(threeJSScene, controlsScene) {
    ...
    this.addView(threeJSScene, CPlane3View, {layer: 0});
  ...
  snap(mouseScreenPos, mouseWorldPos) {
    function round(position, roundSize) {
      return new THREE.Vector3(
        decimalAdjust(Math.round(position.x/roundSize)*roundSize, -1),
        decimalAdjust(Math.round(position.y/roundSize)*roundSize, -1),
        decimalAdjust(Math.round(position.z/roundSize)*roundSize, -1));
    }
    ...
    // Compute snape candidates for each snap type. Distance is
