...
class CPlane3View extends View {
  ...
  constructor(model, scene, options) {
    ...
    function round(position, roundSize) {
      return {
        x: decimalAdjust(Math.round(position.x/roundSize)*roundSize, -1),
        y: decimalAdjust(Math.round(position.y/roundSize)*roundSize, -1),
        z: decimalAdjust(Math.round(position.z/roundSize)*roundSize, -1),
      };
    }
    ...
    const _this = this;
    scene.rawEventGenerator.on('mousemove', (event, screenPosition) => {
      const world = toWorldPosition(scene.width, scene.height, scene.camera, screenPosition);
      console.log('world', world);
      const xy = worldPositionToXY(world, scene.camera);
      _this.emit('mousemove', event, snap(xy));
    });
    scene.rawEventGenerator.on('click', function() {
